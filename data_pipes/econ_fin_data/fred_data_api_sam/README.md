...
```bash
fred_data_api_sam$ sam build --use-container
```
...
```bash
fred_data_api_sam$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
fred_data_api_sam$ sam local start-api
fred_data_api_sam$ curl http://localhost:3000/
```
...
```bash
fred_data_api_sam$ sam logs -n HelloWorldFunction --stack-name fred_data_api_sam --tail
```
...
```bash
fred_data_api_sam$ pip install pytest pytest-mock --user
fred_data_api_sam$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name fred_data_api_sam
```
